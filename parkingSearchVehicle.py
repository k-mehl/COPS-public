...
class ParkingSearchVehicle(object):
    ...
    #  @param name Corresponds to the vehicle ID obtained from the route XML file
    #  @param timestep For memorizing the simulation time when a vehicle is created
    def __init__(self, name, timestep=-1001):
        ...
        # information about vehicle activity status, initially vehicle cruises without searching ("phase 1")
        self.activity = VEHICLE_CRUISING
        # information about relevant simulation times; -1001 seems to be used for unset values in SUMO examples
        ...
        # information about the edge of an eventually found parking space in the opposite direction
    ...
    #  @param parkingSpaces Information about (currently all) available parking spaces
    #  @param oppositeEdgeID Contains a dictionary for identification of the current opposite direction edge
    #  @param timestep Information about the current simulation time
    def update(self, parkingSpaces, oppositeEdgeID, timestep=-1001):
        # get all relevant information about the vehicle from SUMO via TraCI calls:
        # get vehicle speed
        ...
        # get information about the current lane (ID, length and vehicle position)
        ...
        # and divide current route into remaining segments ('active') and traversed segments
        ...
        # remove information about the seen parking space on the previously opposite edge
        # (because it is no longer opposite) 
        if self.seenOppositeParkingSpace == self.currentEdgeID:
            self.seenOppositeParkingSpace = ""
        
        # if the vehicle is cruising and has entered the search district, change to search phase 2
        if self.isSearchingVehicle and self.activity == VEHICLE_CRUISING and self.currentRouteIndex >= 1:
        ...
        if self.activity == VEHICLE_SEARCHING: 
            # if parking space is found ahead on current edge, change vehicle status accordingly
            if (timestep >= self.timeBeginSearch+1) and self.lookoutForParkingSpace(parkingSpaces):
            	self.activity = VEHICLE_FOUND_PARKING_SPACE
                # let the vehicle stop besides the parking space
            	traci.vehicle.setStop(self.name, self.currentEdgeID, self.assignedParkingPosition, 0, 2**31 - 1, 0)
                # set the vehicle color to orange to indicate braking in the SUMO GUI
            	traci.vehicle.setColor(self.name,(255,165,0,0))
            # if still searching and an opposite edge exists, look there as well
            if self.activity == VEHICLE_SEARCHING and self.seenOppositeParkingSpace=="" and self.currentEdgeID in oppositeEdgeID:
                self.seenOppositeParkingSpace = self.lookoutForOppositeParkingSpace(parkingSpaces, oppositeEdgeID) 

        # if the vehicle has stopped besides found parking space, basically block the road for a while
        ...
        # twelve seconds after beginning to maneuver into a parking space, 'jump' off the road and release queueing traffic
        if self.activity == VEHICLE_MANEUVERING_TO_PARK and (timestep > (self.timeBeginManeuvering + 12)):
            self.activity = VEHICLE_PARKED
            # for the change between 'stopped' and 'parked' in SUMO, first the issued stop command has to be deleted by 'resume'
            traci.vehicle.resume(self.name)
            # now, we can directly stop the vehicle again, this time as 'parked' (last parameter 1 instead of 0)
            traci.vehicle.setStop(self.name, self.currentEdgeID, self.assignedParkingPosition, 0, 2**31 - 1, 1)
            ...
            print(self.name, "parked after", (self.timeParked - self.timeBeginSearch), "seconds,", traci.vehicle.getDistance(self.name), "meters.")
            

    ## Lookout for available parking spaces by checking vehicle position information against the 'map' of existing parking spaces
    #  @param parkingSpaces Information about all parkingSpaces in the network
    def lookoutForParkingSpace(self, parkingSpaces):
        if self.speed > 0.0:
            # for all existing parking spaces, check if there is one available within the assumed viewing distance of the driver
            for parkingSpace in parkingSpaces:
                # only consider parking spaces on the current edge
                if parkingSpace.available and (parkingSpace.edgeID==self.currentEdgeID):
                    # only consider parking spaces which are
                    # - far away enough so that the vehicle can safely stop (otherwise SUMO will create an error)
                    # - within a distance of max. 30 meters in front of the vehicle
                    if (parkingSpace.position-self.currentLanePosition >12.0) and (parkingSpace.position-self.currentLanePosition <30.0):
                        # found parking space is assigned to this vehicle
                        # (from now, parking space is no longer available to other vehicles)
    ...
    def lookoutForOppositeParkingSpace(self, parkingSpaces, oppositeEdgeID):
        if self.speed > 0.0:
            ...
            for parkingSpace in parkingSpaces:
                ...
                if parkingSpace.available and (parkingSpace.edgeID==oppositeEdgeID[self.currentEdgeID]):
                    if (parkingSpace.position < self.currentLaneLength-self.currentLanePosition):
                        if (parkingSpace.position > self.currentLaneLength-(self.currentLanePosition+30.0)):
                            # if an opposite parking space has been found,
                            # insert a loop to the active route (just once back and forth)
                            self.activeRoute.insert(0, oppositeEdgeID[self.currentEdgeID])
                            self.activeRoute.insert(0, self.currentEdgeID)
                            # communicate the modified active route to the vehicle via TraCI
